...
namespace ML_Accord_KNN
{
    public class PancakeKNN_V3
    {
        ...
        public void FullTestToGetBestKValue(int testRounds = 200)
        {
            var testItem = GetTestItem();
            var bestParam = new BestParam();
            var lessSerialLostParam = new BestParam() { serialLost = 50 };
        ...
        public KNNResult KNNDecide_0123(RoundData testData = null, int kValue = 500)
        {
            if (testData == null)
            {
                testData = GetTestItem();
            }
            if (dataList.Count <= kValue)
            {
            ...
            if (inputs.Length < kValue)
            {
                return new KNNResult();
            }

            //Get result
            KNNResult result = KNNDecide(kValue, testData);
            return result;
        }
        ...
        {
            RoundData testItem = GetTestItem();
        ...
        public KNNResult KNNDecide(int k, RoundData testData)
        {
            ...
            var kNearestNeighbors = knn.Learn(inputs, outputs);
        ...
        private void BuildDatas(int lastCount, int maxRoundID = 999999)
        {
