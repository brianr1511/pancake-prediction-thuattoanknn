...
namespace Pancake_Pridction_KNN
{
    public partial class MainForm : Form
    {
        ...
        private async void MainForm_Load(object sender, EventArgs e)
        {
            ...
            Log.ShowInUI("Game đã chạy thành công.");
            game.OnEvent += (ss, ee) =>
             {
                 switch (ee.Type)
                 {
                     case GameController.ConEventType.NewRoundStarted:
                         {
                             ...
                             _log.Info($"Round mới ID:  {CurrentRoundID}");
        ...
        private void timerForTickDown_Tick(object sender, EventArgs e)
        {
            ...
            label_Balance.Text = $"Số dư: {GameController.balanceBNB:f2}";
            ...
            if (IsThisRoundAutoBetted == false && !string.IsNullOrEmpty(Config.cfg.DbServerName))
            {
                ...
                if (ts.TotalSeconds >= 18 && ts.TotalSeconds <= 20 && DataFeedWorking)
                {
                    if (Math.Abs(chainlinkPriceOffset) >= 0.9)
                    {
                        ...
                        Log.ShowInUI($"Tiền dô tiền dô, chainlinkPriceOffset:{chainlinkPriceOffset:f2}  Bet {side}");
                    }
                }
                ...
                if (ts.TotalSeconds >= 18 && ts.TotalSeconds <= 23)
                {
                    if (DataFeedWorking && CheckingToBet == false)
                    {
                        ...
                        Log.ShowInUI($"RoundID:#{CurrentRoundID}#  10s cuối thay đổi: {kline_10s.change:f3}   chainlinkOffset：{chainlinkPriceOffset:f2} ");
                        IsThisRoundAutoBetted = true;
                        ...
                        try
                        {
                            ...
                            Log.ShowInUI($"RoundID:{knnResult_v30123.RoundID}" +
                                ...
                                $"  Điểm: {knnResult_v30123.Score:f2}" +
                                $"  %Kháng cự: {knnResult_v30123.AgainstPercent:P2}" +
                            ...
                            {
                                Log.ShowInUI("knn_v30123 đưa kết quả bất thường, không đặt round này!");
                        ...
                        if (lastAutoBet != null)
                        {
                            ...
                            {
                                betAmount = (lastAutoBet.Amount * 1) + addtionAmount;
                                bet_Amount_BNB_tbox.Text = $"{betAmount}";
                                Log.ShowInUI($"Ván này thua, ván sau gấp đôi đến chết lên (Tạm thời tắt gấp đôi) {betAmount}");
                            }
                            else if (roundResult == RoundResult.UNKNOW)
                            {
                                if (lastAutoBet.Amount < betAmount * 2 * 2 * 2 * 2)
                                {
                                    betAmount = (lastAutoBet.Amount * 1) + addtionAmount;
                                    bet_Amount_BNB_tbox.Text = $"{betAmount}";
                                    Log.ShowInUI($"Ván này bất ổn, ván sau gấp đôi đến chết (Trung An tạm thời tắt gấp đôi) {betAmount}");
                                }
                                else
                                if (lastAutoBet.Amount > betAmount * 2 * 2 * 2 * 2 * 2)
                                {
                                    ...
                                    Log.ShowInUI($"Ván này thua sml, ván tới reset về lại {betAmount}");
                                }
                                else
                                {
                                    betAmount = (lastAutoBet.Amount) + addtionAmount;
                                    Log.ShowInUI($"Ván này bất ổn, ván sau vẫn sẽ giữ mức {betAmount}");
                                }
                            }
                            else if (roundResult == RoundResult.WIN)
                            {
                                bet_Amount_BNB_tbox.Text = baseAmount.ToString();
                                betAmount = baseAmount;
                                Log.ShowInUI($"Ván này ăn rồi, ván sau về lại mức cược {betAmount}, tham là còn cái nịt.");
                            }
                        }
                        ...
                        if (betSide != BetSide.@null)
                            Log.ShowInUI($"Thuật toán KNN đặt cược {betSide} với {betAmount}");
                    ...
                    {
                        Log.ShowInUI("Server trade dead.");
                    }
                }
            }
        ...
        private RoundResult CheckCanWin(BetSide side)
        {
            ...
            Log.ShowInUI($"chainLink:{chainLink:f3}  Gần nhất:{latest:f3}");
        ...
        private double ShowPriceOffset()
        {
            ...
            label_lastLPTime.Text = $"{(DateTime.Now - lastTimeUpdateLP).TotalSeconds:f0}giây trước";
        ...
        private void GoBNB_WebsocketSubscrib(string Proxy = "")
        {
                        ...
                        if (ee == "trade")
                        {
                            ...
                            if ((trade.time - lastUpdateSec2).TotalSeconds >= 0.3)
                            {
                                ...
                                if (m5KlineStartTime < trade.time)
                                {
                                    ...
                                    m5KlineStartTime = trade.time.AddSeconds(590 - KlineCountdownTimeInSecounds);
                                }
            ...
            webSocket.OnClose += (s, e) =>
            {
                while (true)
                {
                    try
                    {
                        var msg = $"Trade của websocket bị đứt đoạn vì {e.Reason}, đợi kết nối lại.";
                        Log.ShowInUI(msg);
                        _log.Error(msg);
                        Thread.Sleep(40000);
                        webSocket.Connect();
                        ...
                        {
                            Log.ShowInUI("Kết nối lại websocket server thành công");
                            break;
                        }
                    }
                    catch (Exception ex)
                    {
                        _log.Error($"Gặp lỗi khi kết nối lại websocket server. {ex}");
                    }
                }
            };
            ...
            {
                Log.ShowInUI($"Đã kết nối websocket.");

                webSocket.Send(subscribString);
                Log.ShowInUI($"Test thử subscristring trên websocket thành công");
            };
        ...
        private async void toolStripMenuItemRefreshBooks_Click(object sender, EventArgs e)
        {
            ...
            //CheckBNBForBearOrBull
            {
                ...
                label_bnbDown.Text = $"XUỐNG:{countBnbForDown:f1}";
                var countBnbForUp = orders.bids.Where(w => w.price < LastClosePrice).Sum(s => s.vol);
                label_bnbUPUP.Text = $"LÊN:{countBnbForUp:f1}";
            }
        ...
        private async void button_ManualBull_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (slowKDJ != null)
                {
                    ...
                    {
                        Log.ShowInUI($"Đặt Bull lỗi! {ex.Message}");
                    }
            ...
            catch (Exception ex1)
            {
                ...
                Log.ShowInUI($"Đặt Bull lỗi！{ex1.Message}");
            }
            finally
            {
