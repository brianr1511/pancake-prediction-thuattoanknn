...
namespace Pancake_Pridction_KNN.Contract
{
    public partial class GameController
    {
        ...
        public async Task StartPrediction()
        {
            ...
            {
                Log.ShowInUI($"Task làm mới round đã chạy thành công.");
                var oldRoundID = 0;
                ...
                while (true)
                {
                    ...
                    {
                        _log.Debug($"Có lỗi xảy ra khi lấy thông tin ID Round vừa rồi, cụ thể là API CurrentEpochQueryAsync. {ex.Message}");
                    }
                }
            ...
            {
                Log.ShowInUI($"Cập nhật số dư ví thành công.");
                while (true)
                {
                    try
                    {
                    ...
                    {
                        _log.Debug($"Có lỗi trong quá trình lấy số dư ví. {ex.Message}");
                    }
                }
        ...
        private async Task ProcessEndRoundEvent(int roundID)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        _log.Error($"Lỗi ở hàm ProcessEndRoundEvent khi lấy thông tin về round vừa rồi.{ex}");
                    }
                }
                ...
                var roundWin = ((round.BetSide == BetSide.@null || round == null) ? "Chưa đặt" : round.Winner.ToString());
                Log.ShowInUI($"Kết thúc round ID :{oldRoundID}  Giá:{evtPrice:f2}  Kết quả: {result}  Phán đoán: {roundWin}");
                if (roundWin == "False")
                {
                    lostTimes++;
                    if (lostTimes >= 3)
                ...
                if (round != null && round.Winner)
                {
                    try
                    {
                        MainForm.WinerCount++;
                        var faildTimes = 0;
                        Log.ShowInUI($"Chuẩn bị claim phần thưởng round ID:{oldRoundID} ");
                        ...
                        Log.ShowInUI($"Claim phần thưởng thành công...");
                    }
                    catch (Exception ex)
                    {
                        Log.ShowInUI($"Lỗi Claim, quá giờ! {ex}");
                    }
                }
            }
            catch (Exception e)
        ...
        public async Task<bool> ManualBet(BetSide betSide, double betAmountInBNB, bool QuickBet = false)
        {
            ...
            {
                Log.ShowInUI("Cược thất bại! Vòng cược hiện tại là 0!");
                return false;
            }
            ...
            if (betBull)
            {
                ...
                {
                    Log.ShowInUI($"Đặt Bull thất bại :{ex.Message}");
                    side = BetSide.@null;
                }
            }
            else
            {
                ...
                {
                    Log.ShowInUI($"Đặt Bear thất bại :{ex.Message}");
                    side = BetSide.@null;
                }
            }
            ...
            {
                Log.ShowInUI($"Bet {(betBull ? "Đặt Bull" : "Đặt Bear")} thất bại! BNB:{betAmountInBNB}");
                return false;
            }
            else
            {
                Log.ShowInUI($"Bet {(betBull ? "Đặt Bull" : "Đặt Bear")} thành công! Phí gas đã xài: {gasUsed}  BNB:{betAmountInBNB}");
        ...
        public async Task<bool> ClaimPrize(int roundID = 0)
        {
            ...
            if (claim.Epochs.Count > 0)
            {
                ...
                {
                    Log.ShowInUI($"Claim thành công! Gas đã xài: {claimFunctionTxnReceipt.GasUsed}");
                }
                else
                {
                    Log.ShowInUI($"Claim thất bại! Gas đã xài: {claimFunctionTxnReceipt.GasUsed}\r\n Trạng thái: {claimFunctionTxnReceipt.Status}");
                }
                return true;
            }
            else
                Log.ShowInUI($"Không có phần thưởng để claim.");
            return false;
        }
